...
struct ShaderUtils {
    ...
    texture_fragment_shader(const FragmentShader& fs) {
        Vector3 finalColor{};
        if (fs.texture) {
            finalColor = fs.texture->getColor(fs.uv[0], fs.uv[1]) / 255;
        }
        else {
            finalColor = fs.color;
        }
    ...
    bump_fragment_shader(const FragmentShader& fs) {
        ...
        numberType dU = kh * kn * (fs.texture->getColor(u + 1.0 / w, v).norm2() - fs.texture->getColor(u, v).norm2());
        numberType dV = kh * kn * (fs.texture->getColor(u, v + 1.0 / h).norm2() - fs.texture->getColor(u, v).norm2());
    ...
    displacement_fragment_shader(const FragmentShader& fs) {
        ...
        Vector3 ln = Vector3{-dU, -dV, 1.0};
        normal = (TBN * ln).normalize();
        point += (kn * normal * fs.texture->getColor(u, v).norm2());
